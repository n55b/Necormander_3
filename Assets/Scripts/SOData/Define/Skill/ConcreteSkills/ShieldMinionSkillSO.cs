...
public class ShieldMinionSkillSO : MinionSkillSO
{
    ...
    public override void ExecuteSkill(Transform user, Transform target = null)
    {
        ...
        {
            player.StartCoroutine(MinionJumpRoutine(player, targetPos, shieldMinions));
        }
        else
        {
    ...
    private IEnumerator MinionJumpRoutine(PlayerController player, Vector2 targetPos, List<AllyController> minions)
    {
        float elapsed = 0f;
        ...
        float radius = 1.5f;
        float angleStep = minions.Count > 0 ? 360f / minions.Count : 0f;

        for (int i = 0; i < minions.Count; i++)
        {
            mStartPos.Add(minions[i].transform.position);
        ...
        for (int i = 0; i < minions.Count; i++)
        {
            ...
            if (m != null && !m.Stats.Health.IsDead)
            {
                ...
                {
                    BaseHitBox box = Instantiate(hitBoxPrefab, m.transform.position, Quaternion.identity);
