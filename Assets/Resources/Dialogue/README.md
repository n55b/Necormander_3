...
대사는 이 폴더의 **CSV** 로 관리한다. 화면은 하단 1/3 이 대사창, 상단 2/3 가 초상화 5칸.
...
| `cast` | 무대에 세울 캐릭터 키, `\|` 로 구분 (최대 5) | **이전 줄의 무대를 그대로 유지** |
...
캐릭터 키 뒤에 `/표정` 을 붙인다. `cast` 와 `speaker` 양쪽에서 쓸 수 있다.
...
`layout_test` 는 스페이스를 다섯 번 누르면 **1명 → 5명 배치**가 차례로 나온다.

### 플레이 안 켜고 — `Tools/Dialogue/4. 레이아웃 미리보기 토글`

씬을 연 채로 메뉴를 누르면 대사창이 켜지고 5칸이 견본으로 채워진다.
...
5칸 중 **가운데로 모은다.** `cast` 에 적은 순서대로 채워진다.
